...
echo [1/7] Installing root desktop dependencies...
call :run_npm install --legacy-peer-deps
if errorlevel 1 goto :build_failed

echo [2/7] Installing frontend dependencies...
call :run_npm --prefix frontend install --include=dev --legacy-peer-deps
if errorlevel 1 goto :build_failed
...
echo [4/7] Building Python backend sidecar...
...
echo [6/7] Copying frontend assets into Electron output...
call :run_npm run copy:frontend
if errorlevel 1 goto :build_failed

echo [7/7] Building Windows installer and portable zip...
